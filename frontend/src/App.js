...
function App() {
  ...
  const handleSelectProject = (project) => {
    ...
    if (project.configuration) {
      ...
      if (config.products && Array.isArray(config.products)) {
        ...
        // Sync legacy configuration with first product AND Business Document Information
        if (config.products.length > 0) {
          setConfiguration({
            ...config.products[0],
            // Business Document Information (stored at config root level)
  ...
  const calculateNextStep = (itemsToSkip = 0) => {
    ...
    const totals = calculateCurrentProductTotals(activeSerialProductIndex);
